...
clk_1hz_subtaskD clk3 (CLOCK, master_switch, master_stop); //subtask_D_led);                                // Clock instatiations for Subtask D
    
 // Subtask C -- tracks the current symbol
 reg  [1:0] current_symbol = 0; 
    ...
    if(master_stop == 1) // Checks if master_stop was asserted, if yes, 
        ...
        begin
            if(subtaskA_done_flag == 0)
             ...
             else if(subtaskA_done_flag == 1 & unlocked_led == 0)
...
always @ (master_switch or unlocked_led or segment_count or segment_count_L or segment_count_U or segment_count_R) begin
    if(master_stop == 1) 
        begin
            begin an <= 4'b0000; seg <= 7'b100_0001; end
        end
        
    else 
        begin
            if (subtaskA_done_flag == 0)
                begin an <= 4'b1111; seg <= 7'b111_1111; end  // edge case: unlocked_led ==0, but master_switch on then offend         
                            ...
                            if(sw[2]==1)
                                begin
                                    case(clock_100hz_status_count)
                                        0: begin an <= 4'b1110; seg <= 7'b010_0111; current_symbol <= 0; end // c
                                        1: begin an <= 4'b1101; seg <= 7'b100_1111; current_symbol <= 1;end // l 
                                        2: begin an <= 4'b1011; seg <= 7'b110_0011; current_symbol <= 2;end // u
                                        default: begin an <= 4'b1001; seg <= 7'b111_0111; end // error signal
                                    endcase
                                end
                            else if(sw[1]==1)
                                begin
...
//always @ (clock_100hz_status_count) // debugging purposes
//begin
...
/* -------------------- Subtask D -------------------*/
//assign subtask_D_led = master_stop; // debugging purposes
