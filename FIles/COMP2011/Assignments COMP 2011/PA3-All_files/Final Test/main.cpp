...
int main()
{
    ...
    if(choice == 2) //play the game
    {
        ...
        int width = 11;  //map width
        int height = 11; //map height
        Node ***map = generateMap(width, height);

        addThing(map, width, height, 0, 0, HERO, 1);
        addThing(map, width, height, 0, 2, SWORD, 3);
        addThing(map, width, height, 3, 10, DRAGON, 1);
        
            addThing(map, width, height, 9, 3, DAGGER, 2);

            addThing(map, width, height, 3, 4, SKELETON, 1);
            addThing(map, width, height, 7, 2, SKELETON, 3);
            addThing(map, width, height, 3, 4, SKELETON, 1);
            addThing(map, width, height, 7, 2, DRAGON, 3);

            addThing(map, width, height, 3, 3, MACE, 1);
            addThing(map, width, height, 0, 7, MACE, 1);
            addThing(map, width, height, 2, 7, MACE, 5);
            addThing(map, width, height, 3, 7, MACE, 1);
            addThing(map, width, height, 2, 7, MACE, 1);

            addThing(map,width,height,2,0,DAGGER,1);//
            addThing(map,width,height,2,0,DAGGER,1);//
            ...
            removeThing(map,width,height,1,0,MACE,2);
            removeThing(map,width,height,3,3,MACE,2);
