...
bool addThing(Node*** map, int width, int height, int x, int y, Thing thing, int quantity)
{
    ...
    if (getLinkedListLength(map[y][x]) == 0 || map[y][x] == nullptr){

        Node* newnode = ll_create(thing,quantity);
        newnode->next = map[y][x];
        map[y][x] = newnode;
        return;
    }
    ...
    Node* p = map[y][x];
    for (int position = 0; position < getLinkedListLength(map[y][x])-1 && p->next != nullptr;p = p->next, ++position);

    // STEP 3,4: Insert the new node between
    // the found node and the next node
    newnode->next = p->next; // STEP 3
    p->next = newnode; // STEP 4
