    ...
    else
    {
        if(map[y][x]!=NULL){
            ...
            else
            {
                ...
                if(newnode != ll_search(temp,thing)&&getLinkedListLength(map[y][x])>1){
...
bool removeThing(Node*** map, int width, int height, int x, int y, Thing thing, int quantity)
{
    ...
    if(current!= nullptr){
        if(current == map[y][x]){
            cout<<"current == map[y][x]"<<endl;
            if(map[y][x]->quantity == quantity && getLinkedListLength(map[y][x])==1){
                map[y][x]->quantity = map[y][x]->quantity - quantity;
