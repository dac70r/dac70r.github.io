...
void print_addition_carry_out(int a, int b)
{
	int count = 0;
	int length;
	int a1 = get_min_bits_length(a);
	int b2 = get_min_bits_length(b);
	length = (a1>b2)?a1:b2;
	int x,y;
	...
	if((x>=0)&&(y>=0)){
		while(count<length){
			...
			x = get_bit_positive(a,count);
			y = get_bit_positive(b,count);
			...
			else if(((x==1)&&(y==0)&&(n==1))||((x==0)&&(y==1)&&(n==1))||((x==0)&&(y==1)&&(n==1)))
			{
				cout<<1;
				n++;
				n%=2;
			...
			else if(((x==0)&&(y==0)&&(n==1))||((x==0)&&(y==1)&&(n==0))||((x==1)&&(y==0)&&(n==0)))
			{
				cout<<1;
			}
