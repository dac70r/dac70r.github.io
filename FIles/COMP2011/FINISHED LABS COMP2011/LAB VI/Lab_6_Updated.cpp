...
bool is_dangerous(char maze[], int x, int y, int width, int height) {
    ...
    if(position==tom+1 && (tom+1)%width!=0)
    return true;

    if(position==tom+1 && (tom+1)%width==1)
    return false;

    if(position==tom+width)
    return true;
    
    if(position==tom-width)
    return true;

    if(position==tom-width-1)
    return true;

    if(position==tom+width-1)
    return true;

    if(position==tom+width+1 && (tom+width+1)%width !=0)
    return true;

    if(position==tom+width+1 && (tom+width+1)%width ==1)
    return false;
...
bool find_safe_path(char maze[], bool path[], int x, int y, int exit_x, int exit_y, int width, int height) {
    ///base case
    if(is_exit(x,y,exit_x,exit_y)==true){
        //update bool path
        path[y*width+x]=true;
        return true;
    }

    if(is_dangerous(maze,x,y,width,height)==false && is_valid(x,y,width,height) == true && is_wall(maze,x,y,width,height)==false )
    {
        ...
        if(find_safe_path(maze,path,x,y+1,exit_x,exit_y,width,height) == true){
        path[y*width+x]=true;
        return true;
        }
...
bool find_safe_path_with_cheese(char maze[], bool path[], int cheese_x, int cheese_y, int width, int height)  {
    ...
    find_safe_path(maze,path,0,0,cheese_x,cheese_y,width,height);
    bool status = find_safe_path(maze,path,cheese_x,cheese_y,width-1,height-1,width,height);
